...
function generateTestData() {
    ...
    let hash = crypto.createHmac('sha512', salt).update("asdf").digest("base64");
    ...
    for (let i = 1; i <= 30; i++) {
        ...
        db.conn.query("INSERT INTO users SET ?", data, (err, result) => {
            ...
            if (result && result.affectedRows > 0 && !err) {
                ...
                db.conn.query('INSERT INTO mwo.items (itemName, itemDescription, itemPhoto, itemPriceCategory, itemCategory, itemUserId, itemStatus) VALUES ?;', [insert], function(err, res, fields) {
